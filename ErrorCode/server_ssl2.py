...
service_on = 1## for service status

global SIP_SERVER_IP,SIP_SERVER_PORT
global LOCAL_PC_RTP_PORT
...
#        self.path = path  
    ...
    def sendMessage(self,message):  
        ...
        #before that, message is unicode format. After encoding, Now is str format.  
        message_utf_8 = message.encode('utf-8') 
...
#####    added by Amandashan end
    ...
    def send_INVITE_response_confroom(self,sock, msg_info):
        ...
        ok_response +="Record-Route: <sip:%s@%s:5060;transport=udp;lr>\r\n"%(msg_info.sipid_number,msg_info.local_ip_number)
...
#             ok_response += "Session-Expires: %s\r\n" %msg_info.sessionExpires
    ...
    def send_register_response_200(self,sock, buffer, msg_info): 
        ...
        if self.compare_string(msg_info.snd_via_header, 'NONE') != 0:
            ok_response += msg_info.snd_via_header + "\r\n"
        ok_response += "Contact: <sip:reg@%s;rtcweb-breaker=no;click2all=no;transport=ws;+g.oma.sip-im;ovid=asdfghjkl;ri=%s;options=010>;expires=600\r\n"%(msg_info.invalid,msg_info.local_ip_number)
        if msg_info.hastotag == True:
            ok_response += "To :%s"%msg_info.to_header + "\r\n"
        else:
            ok_response += "To: %s"%msg_info.to_header + ";tag=201203271"+"\r\n"   
        ...
        ok_response += "Call-ID: %s\r\n" % msg_info.call_id
        ok_response += "CSeq: %s\r\n" %msg_info.cseq
        ok_response += "Path: <sip:127.0.0.1:6060;transport=tcp;lr;ovid=asdfghjkl>\r\n"
        ok_response += "Path: <sip:lkjhgfdsa@%s:10080;transport=ws;lr;ovid=asdfghjkl;ob>\r\n"%msg_info.local_ip_number
        ok_response += "Server: python\r\n"
        ok_response += "X-GS-SERVER-ID: %s"%msg_info.local_ip_number
...
#            msg_info.user_agent = 'windows'
        ...
        if flag == 1:
            park_content = "<?xml version=\"1.0\" encoding=\"utf-8\"?><conference-info refresh=\"%s\" entity=\"%s\" state=\"partial\" s=\"1101100001110000\" options=\"00\"><users><user entity=\"%s\" state=\"full\" display=%s ua=\"Grandstream WebRTC/firefox 41.0\" type=\"5\" s=\"0111\" options=\"00\"/></users></conference-info>" %(self.refreshTime,msg_info.meeting_id_to_user,str(msg_info.sipid_number),msg_info.display_name)
...
#        notify_request += "X-Gs-Server-Id: %s\r\n"%msg_info.local_ip_number
    ...
    def send_av_invite(self,sock,msg_info):
        ...
        sdp_content = "v=0\r\n"
        sdp_content += "o=root 1100670332 1100670333 IN IP4 {localIP}\r\n"
        sdp_content += "s=GrandStream X-Server 0.0.0.41 (M)\r\n"
        sdp_content +="c=IN IP4 {localIP}\r\n"
        sdp_content +="b=CT:384\r\n"
        sdp_content +="t=0 0\r\n"
        sdp_content +="m=audio 48888 RTP/SAVPF 9 0 8 3\r\n"
        sdp_content +="a=rtpmap:9 G722/8000\r\n"
        sdp_content +="a=rtpmap:0 PCMU/8000\r\n"
        ...
        sdp_content +="a=rtpmap:3 GSM/8000\r\n"
        sdp_content +="a=maxptime:150\r\n"
        sdp_content +="a=ice-ufrag:6fce635a2975c3242b8d15c565f6765e\r\n"
        sdp_content +="a=ice-pwd:26be079e1696e5ae302cd019190629a7\r\n"
        sdp_content +="a=candidate:Hacacac06 1 UDP 2130706431 {localIP} 48888 typ host\r\n"
        sdp_content +="a=candidate:Hacacac06 2 UDP 2130706430 {localIP} 48889 typ host\r\n"
        sdp_content +="a=connection:new\r\na=setup:passive\r\n"
        sdp_content +="a=fingerprint:sha-256 67:04:9D:03:6D:64:FA:31:2C:83:B1:4F:90:8F:E4:33:5D:F1:A4:F8:6C:59:8D:38:B7:DA:94:7F:92:73:52:BE\r\n"
        sdp_content +="a=sendrecv\r\n"
        sdp_content +="m=video 48970 RTP/SAVPF 105 100\r\n"
        sdp_content +="a=ice-ufrag:0249719f47a3474975bde9d001fa6e66\r\n"
        sdp_content +="a=ice-pwd:6778a62517c87da67d285e2b4bdff28b\r\n"
        sdp_content +="a=candidate:Hacacac06 1 UDP 2130706431 {localIP} 48970 typ host\r\n"
        sdp_content +="a=candidate:Hacacac06 2 UDP 2130706430 {localIP} 48971 typ host\r\n"
        sdp_content +="a=connection:new\r\n"
        sdp_content +="a=setup:actpass\r\n"
        sdp_content +="a=fingerprint:sha-256 67:04:9D:03:6D:64:FA:31:2C:83:B1:4F:90:8F:E4:33:5D:F1:A4:F8:6C:59:8D:38:B7:DA:94:7F:92:73:52:BE\r\n"
        sdp_content +="a=rtpmap:105 H264/90000\r\n"
        sdp_content +="a=fmtp:105 profile-level-id=42e01f;level-asymmetry-allowed=1;packetization-mode=1\r\n"
        sdp_content +="a=rtpmap:100 VP8/90000\r\n"
        sdp_content +="a=rtcp-fb:* ccm fir\r\n"
        sdp_content +="a=sendrecv\r\n" 
        sdp_content=sdp_content.format(localIP=msg_info.local_ip_number)
        
        if str(msg_info.meeting_pw) == "NONE" or msg_info.meeting_pw == "":
            invite_request = "INVITE sip:%s@%s SIP/2.0\r\n"%(msg_info.sipid_number, msg_info.local_ip_number)
        else:
            invite_request = "INVITE sip:%s*%s@%s SIP/2.0\r\n"%(msg_info.sipid_number, msg_info.meeting_pw, msg_info.local_ip_number)
        if msg_info.protocol == 'udp':
            invite_request += "Via: SIP/2.0/UDP %s:%d;branch=z9hG4bK12345678%x;rport;alias"%(msg_info.local_ip_number,msg_info.local_port_number, random.randint(0, 10000)) + "\r\n"
            invite_request += "Contact: <sip:" + msg_info.sipid_number + "@" + msg_info.local_ip_number + ":" + "%d"%msg_info.local_port_number + ";transport=udp>\r\n"
        else:
            ...
            invite_request += "Contact: <sip:%s@%s:5062;transport=tcp>\r\n"%(str(msg_info.meeting_id_to_user), str(msg_info.local_ip_number))
        if msg_info.hastotag == True:
            if msg_info.meeting_pw == "NONE" or msg_info.meeting_pw == "":                
                invite_request += "To: <sip:%s@%s:%s>;tag=%s\r\n"%(msg_info.sipid_number, msg_info.local_ip_number, msg_info.local_port_number, self.av_invite_totag)
            else:
                invite_request += "To: <sip:%s*%s@%s:%d>;tag=%s\r\n"%(msg_info.sipid_number, msg_info.meeting_pw, msg_info.local_ip_number, msg_info.local_port_number, self.av_invite_totag)
    
        else:
            if msg_info.meeting_pw == "NONE" or msg_info.meeting_pw == "":
                invite_request += "To: <sip:%s@%s>;tag=%s\r\n"%(msg_info.sipid_number, msg_info.local_ip_number,self.av_invite_totag)
            else:
                invite_request += "To: <sip:%s*%s@%s>;tag=%s\r\n"%(msg_info.sipid_number, msg_info.meeting_pw, msg_info.local_ip_number,self.av_invite_totag)
    
            
        invite_request += "From: <sip:%s@%s>;tag=%s\r\n"%(msg_info.meeting_id_to_user, msg_info.local_ip_number, "201203271")
        if self.compare_string(msg_info.authorization_header, 'NONE') != 0:
            invite_request += "Authorization: %s\r\n"%msg_info.authorization_header;
        elif self.compare_string(msg_info.proxy_authorization_header, 'NONE') != 0:
            invite_request += "Proxy-Authorization: %s\r\n"%msg_info.proxy_authorization_header;
        ...
        if self.compare_string(msg_info.minSE, 'NONE') != 0:
            invite_request += "Min-SE: %s\r\n" %msg_info.minSE
            
        invite_request += "Call-ID: %s\r\n"%self.call_id
        invite_request += "CSeq: 103 INVITE\r\n"
        invite_request += "Max-Forwards: 10\r\n"
        invite_request += "Session-Expires: 600;refresher=uas\r\n"
        invite_request +="Min-SE: 600\r\n"
        invite_request += "X-Info: SIP re-invite (External RTP bridge)\r\n"
        invite_request += "X-Channel-Conf-Id: %s\r\n"%msg_info.meeting_id_to_user
        invite_request +="X-Channel-User-Id: %s\r\n"%msg_info.sipid_number
        invite_request += "User-Agent: GrandStream X-Server 1.0.2.6 (M)\r\n"

        invite_request += "Supported: replaces,timer\r\n"
        invite_request += "Allow: INVITE,ACK,CANCEL,OPTIONS,BYE,REFER,SUBSCRIBE,NOTIFY,INFO,PUBLISH,MESSAGE,UPDATE\r\n"
        invite_request += "Content-Type: application/sdp\r\n"
#         invite_request += "Accept: application/sdp, application/dtmf-relay\r\n"
